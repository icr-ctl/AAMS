...
def send_pi_data(temp, pktnum, thousands):
    # data = int(data)
    # Encode payload as bytes
    # data_pkt[0] = (data >> 8) & 0xff
    # data_pkt[1] = data & 0xff
    # Send data packet
    cpu_temp = (temp).to_bytes(2, byteorder='big')
    ...
    data_pkt = cpu_temp + pktnumber + thous
    print(data_pkt)
...
while True:
    ...
    cpu_temp = int(float(temp.replace("temp=","").replace("'C", "")))
    pktnum += 1
    print("Packet number: " + str(pktnum))
    ...
    time.sleep(0.5)
    send_pi_data(cpu_temp, pktnum, thousands)
    time.sleep(3.5)
